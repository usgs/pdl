...
public class JsonNotificationIndexTest {
  ...
  private JsonNotificationIndex index = null;
  ...
  public void setupEnvironment() {
    try {
      ...
      Config c = new Config();
      c.setProperty("url", "jdbc:sqlite:" + indexfile);
      ...
      // Create the index, and startup to create schema
      index = new JsonNotificationIndex();
      index.configure(c);
      index.startup();
      ...
      _query_allNotifications = connection
          .prepareStatement("SELECT * FROM "
              + JsonNotificationIndex.DEFAULT_TABLE);
  ...
  private static Notification createRandomNotification(String source,
    ...
    try {
      ...
      if ((time % 2) == 0) {
        n = new URLNotification(id, d, null, new URL(url));
      } else {
        n = new DefaultNotification(id, d, null);
      }
